...
func (c *Client) Upload(local string, remote string) (err error) {
	...
	if err != nil {
		...
		// return errors.New("sftp: Jumped Upload[" + local + "]) ,local file not found or yaml error!")
	}
	if info.IsDir() {
...
func (c *Client) Download(remote string, local string) (err error) {
	if c.IsNotExist(strings.TrimSuffix(remote, "/")) {
		return errors.New("sftp: remote file not found ,jumped download this file[" + remote + "]")
	}
	if c.IsDir(remote) {
